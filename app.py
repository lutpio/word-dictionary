...
def detail(keyword):
    ...
    # api_key = "25b29491-bc31-44e7-8fa7-8d5ebd167b58"
    url = f"https://www.dictionaryapi.com/api/v3/references/collegiate/json/{keyword}?key={api_key}"
    response = requests.get(url)
    definitions = response.json()
    if not definitions:
        return render_template("error.html", word=keyword, definitions="kosong")
...
def save_word():
    ...
    return jsonify({"result": "success", "msg": f"word { word } saved"})
...
def save_ex():
    ...
    return jsonify(
        {"result": "success", "msg": f"'{example}' untuk kata {word}, tersimpan"}
    )
...
def delete_ex():
    ...
    return jsonify({"result": "success", "msg": f"{word} terhapus"})
